...
const fundFederation = require('./fundFederation');
...
const mainchainFederation = getMainchainFederation(mainKeys);
const sidechainFederation = getSidechainFederation(sideKeys, sideConfig);

run({ mainchainFederation, sidechainFederation, config, sideConfig });

function getMainchainFederation(keys) {
    let federation = [];
    if (keys && keys.length) {
        keys.forEach((key, i) => {
            ...
            federation.push(federator);
        });
    } else {
        let federator = new Federator(config, log4js.getLogger('VALIDATOR'));
        federation.push(federator);
    }
    return federation;
}

function getSidechainFederation(keys, sideConfig) {
    let federation = [];
    if (keys && keys.length) {
        keys.forEach((key, i) => {
            let federator = new Federator({
...
async function run({ mainchainFederation, sidechainFederation, config, sideConfig }) {
    logger.info('Starting transfer from Mainchain to Sidechain');
    await transfer(mainchainFederation, sidechainFederation, config, 'MAIN', 'SIDE');
    logger.info('Completed transfer from Mainchain to Sidechain');

    logger.info('Starting transfer from Sidechain to Mainchain');
    await transfer(sidechainFederation, mainchainFederation, sideConfig, 'SIDE', 'MAIN');
    logger.info('Completed transfer from Sidechain to Mainchain');
}

async function transfer(originFederation, destinationFederation, config, origin, destination) {
    try {
        let data = '';
        ...
        // Start origin federation with delay between them
        logger.debug('Fund federator wallets');
        let federatorKeys = mainKeys && mainKeys.length ? mainKeys : [config.privateKey];
        await fundFederation(config.sidechain.host, federatorKeys, config.sidechain.privateKey, destinationWeb3.utils.toWei('1'));

        await originFederation.reduce(function(promise, item) {
        ...
        if(destinationTokenAddress == '0x0000000000000000000000000000000000000000') {
            logger.error('Token was not voted by federation');
            process.exit();
        }
        ...
        await fundFederation(config.mainchain.host, federatorKeys, config.mainchain.privateKey, originWeb3.utils.toWei('1'));

        // Start destination federation with delay between them
        await destinationFederation.reduce(function(promise, item) {
            return promise.then(function() { return item.run(); })
        }, Promise.resolve());
